...
export const saveToFavourites = async (user, data) => {
  if (!data?.favourites?.includes(user?.uid)) {
    ...
    await updateDoc(docRef, {
      ...
      .then(() => toast.success("Removed From Favourite"))
      .catch((err) => toast.error(`Error : ${err.message}`));
  }
};
...
export const getSavedResume = async (uid) => {
  return new Promise((resolve, reject) => {
    const templateQuery = query(collection(db, "users", uid, "resumes"));
